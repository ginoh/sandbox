...
初回のデプロイはカナリアのアップグレード Step や分析はスキップされてすぐに 100%までスケールする。
...
// Rollout が目的の状態に達する前に安定版のマニフェストを適用するとロールバックとして検出され、Replicaset の作成は行われない
$ kubectl argo rollouts set image rollouts-demo rollouts-demo=localhost/argoproj/rollouts-demo:yellow
```
